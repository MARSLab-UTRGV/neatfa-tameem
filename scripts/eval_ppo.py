...
def main():
    ...
    # Use last saved PPO checkpoint by default
    ckpt = os.path.join(os.path.dirname(__file__), "runs", "ppo_final.pt")
    if not os.path.exists(ckpt):
        print(f"PPO checkpoint not found: {ckpt}")
        print("Available PPO checkpoints:")
    ...
    try:
        ...
        steps = 0
        ep_return = 0.0
        print("Starting PPO evaluation loop...")
        
        while True:
            action = select_action(model, np.array(obs, dtype=np.float32), device)
            ...
            print(f"  Reward: {reward:.3f}, Total: {ep_return:.3f}, Terminated: {terminated}, Truncated: {truncated}")
            
            time.sleep(0.02)  # slow down for visualization (~50 FPS)
            
            if terminated or truncated:
                print(f"Episode done: steps={steps} return={ep_return:.2f}")
                break
